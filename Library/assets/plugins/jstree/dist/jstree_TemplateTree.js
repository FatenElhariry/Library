        ...
        items: function (o, cb) { // Could be an object directly
            return {
                ...
                "remove": {
                    ...
                    "action": function (data) {
                        ...
                        if (Math.floor(id) == id && $.isNumeric(id)) {
                            ...
                            var Data = { 'id': id };
                            $.ajax({
                                url: ROOT + "Manage/RemoveSpecificGroupTemplate",
                                ...
                                success: function (e) {
                                    ...
                                    if (e == 1) {
                                        ...
                                        $("#Notification_BTN_Action").attr("title", "إدارة مجموعات وتفاصيل القوالب");
                                        $("#Notification_BTN_Action").attr("msg", "فشل الحذف, هذا النوع يحتوى على أنواع اخرى بداخله");
                ...
                "EditData": {
                    ...
                    "separator_after": false,
                    "_disabled": false, //(this.check("rename_node", data.reference, this.get_parent(data.reference), "")),
                    "label": "تعديل الإسم بالإنجليزية",
                    "action": function (data) {
                        var inst = $.jstree.reference(data.reference),
							obj = inst.get_node(data.reference);

                        var id = obj.id;
                        var title = $.jstree.reference('#tree_16')._model.data[id].li_attr.title;

                        if (Math.floor(id) == id && $.isNumeric(id)) {
                            $("#Loc_Title_ID").attr("name", id);
                            $("#Loc_Title_ID").val(title);
                            $("#MyPOPUPNameinEn").click();
                        }
                        else {
                            $("#Notification_BTN_Action").attr("ntype", "error");
                            $("#Notification_BTN_Action").attr("title", "إدارة مجموعات وتفاصيل القوالب");
                            $("#Notification_BTN_Action").attr("msg", "من فضلك قم بالحفظ اولا لتتمكن من تعديل الإسم بالإنجليزية");
                            $("#Notification_BTN_Action").click();
                        }
